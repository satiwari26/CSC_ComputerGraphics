...
bool WindowManager::init(int const width, int const height)
{
	...
	windowHandle = glfwCreateWindow(width, height, "Gotham City", nullptr, nullptr);
	if (! windowHandle)
	{
		glfwTerminate();
		return false;
	...
	glfwSetCursorPosCallback(windowHandle, cursor_PosCallback);
...
void WindowManager::cursor_PosCallback(GLFWwindow* window, double xpos, double ypos)
{
    if (instance && instance->callbacks)
    {
